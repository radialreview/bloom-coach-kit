...
| 8a | Assistant names one of its notes but the link opens nothing ("couldn't find this file") | Set up before the memory move, so its notes still sit outside the coach's folder where links can't resolve. Flag Mike — the fix is a one-time move of `agent-memory/<slug>` into the folder as `memory/` (setup skill, step 3). The notes themselves are fine; until then have the assistant paste the text instead of linking. |
